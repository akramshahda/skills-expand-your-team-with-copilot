document.addEventListener("DOMContentLoaded", () => {
    ...
    Object.entries(allActivities).forEach(([name, details]) => {
      ...
      if (currentDifficulty) {
        if (currentDifficulty === "all-levels") {
          // Show only activities with no specified difficulty
  ...
  function renderActivityCard(name, details) {
    ...
    if (details.difficulty && difficultyColors[details.difficulty]) {
      const diffColor = difficultyColors[details.difficulty];
      difficultyHtml = `
        <span class="difficulty-badge" style="background-color: ${diffColor.bg}; color: ${diffColor.text}">
          ${details.difficulty}
        </span>
      `;
    }
